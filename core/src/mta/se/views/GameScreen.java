...
public class GameScreen implements Disposable {
	...
	public transient GameListener listener;
	...
	private void populate () {
		...
		final Texture shipTexture = new Texture(Gdx.files.internal("data/ship2.png"), Format.RGB565, true);
		...
		((ColorAttribute)blockModel.materials.get(0).get(ColorAttribute.Diffuse)).color.set(1, 1, 0, 0.5f);
		blockModel.materials.get(0).set(new BlendingAttribute(GL20.GL_SRC_ALPHA, GL20.GL_ONE_MINUS_SRC_ALPHA));

		shotModel.materials.get(0).set(ColorAttribute.createDiffuse(1, 0, 0, 1f));
		...
		ship.transform.rotate(0, 1, 1, 180);
            ...
            for (int row = 0; row < 4; row++) {
                for (int column = 0; column < 8; column++)
                    if(column!=3 && column!=4){
                        ...
                        invader.transform.rotate(1, 0, 0, 15);
        ...
        for (int shield = 0; shield < 3; shield++) {
            ...
            blocks.add(new Block(blockModel, -10 + shield * 10 + 1, 0, -3));
            blocks.add(new Block(blockModel, -10 + shield * 10 + 1, 0, -2));
            blocks.add(new Block(blockModel, -10 + shield * 10 + 0, 0, -2));
        }
